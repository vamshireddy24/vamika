...
const Card = ({imagePath,title,description,actionArrow,height,width}) => {
  return (
    <div className='flex items-center flex-col p-6'>
        <img className={`h-[${height? height:'240px'}] max-w-[${width? width:'200px'}] max-h-[180px] max-w-[140px] min-h-[180px] min-w-[140px] bg-cover bg-center
         ...
         <div className='flex justify-between items-center'>
          ...
          {actionArrow && <span className='cursor-pointer pr-2 py-2 items-center'><ArrowIcon/></span>}
        </div>
    </div>
  )
}
